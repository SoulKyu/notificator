...
// updateSearchSuggestions updates autocomplete suggestions based on current alerts
func (aw *AlertsWindow) updateSearchSuggestions() {
	// Collect unique values for autocomplete
	suggestions := make(map[string]bool)

	for _, alert := range aw.alerts {
		// Add alert names
		if alertName := alert.GetAlertName(); alertName != "" {
			suggestions[alertName] = true
		}

		// Add teams
		if team := alert.GetTeam(); team != "" && team != "unknown" {
			suggestions[team] = true
		}

		// Add instances
		if instance := alert.GetInstance(); instance != "" {
			suggestions[instance] = true
		}

		// Add severity levels
		if severity := alert.GetSeverity(); severity != "" {
			suggestions[severity] = true
		}

		// Add status values
		if status := alert.Status.State; status != "" {
			suggestions[status] = true
		}

		// Add summary keywords (enhanced to include more words)
		summary := alert.GetSummary()
		words := strings.Fields(summary)
		for i, word := range words {
			if i >= 8 { // Increased from 5 to 8 words
				break
			}
			// Reduced minimum length from 3 to 2 for more suggestions
			if len(word) > 2 && !isCommonWord(word) {
				suggestions[strings.ToLower(word)] = true
			}
		}

		// Add labels as suggestions (if available in alert structure)
		if alert.Labels != nil {
			for key, value := range alert.Labels {
				if key != "" && len(key) > 1 {
					suggestions[key] = true
				}
				if value != "" && len(value) > 2 && !isCommonWord(value) {
					suggestions[value] = true
				}
			}
		}

		// Add annotations as suggestions (if available)
		if alert.Annotations != nil {
			for key, value := range alert.Annotations {
				if key != "" && len(key) > 1 {
					suggestions[key] = true
				}
				// Extract meaningful words from annotation values
				if value != "" {
					annotationWords := strings.Fields(value)
					for _, word := range annotationWords {
	...
	for _, term := range commonTerms {
		suggestions[term] = true
	}

	// Convert to slice
	var suggestionList []string
	for suggestion := range suggestions {
		if len(suggestion) > 1 { // Ensure minimum length
	...
	aw.searchSuggestions = suggestionList
}

// isCommonWord checks if a word is too common to be useful as a search suggestion
func isCommonWord(word string) bool {
	commonWords := map[string]bool{
		...
		"whom": true, "whose": true, "from": true, "into": true, "onto": true,
		"upon": true, "over": true, "under": true, "above": true, "below": true,
	}
