...
func LoginContent(oauthConfig *OAuthConfig) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		if oauthConfig != nil && oauthConfig.DisableClassicAuth {
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 7, "<div class=\"mt-6 text-center\"><p class=\"text-sm text-gray-600 dark:text-gray-400\">Username/password authentication is disabled. Please use one of the OAuth providers above.</p></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
		}
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 8, "<script>\n\t\t\t\tfunction loginForm() {\n\t\t\t\t\treturn {\n\t\t\t\t\t\tform: {\n\t\t\t\t\t\t\tusername: '',\n\t\t\t\t\t\t\tpassword: '',\n\t\t\t\t\t\t\trememberMe: false\n\t\t\t\t\t\t},\n\t\t\t\t\t\terrors: {},\n\t\t\t\t\t\tmessage: { text: '', type: '' },\n\t\t\t\t\t\tloading: false,\n\t\t\t\t\t\t\n\t\t\t\t\t\tasync submitForm() {\n\t\t\t\t\t\t\tthis.clearErrors();\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\tif (!this.validateForm()) {\n\t\t\t\t\t\t\t\treturn;\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\tthis.loading = true;\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\ttry {\n\t\t\t\t\t\t\t\tconst formData = new FormData();\n\t\t\t\t\t\t\t\tformData.append('username', this.form.username);\n\t\t\t\t\t\t\t\tformData.append('password', this.form.password);\n\t\t\t\t\t\t\t\tif (this.form.rememberMe) {\n\t\t\t\t\t\t\t\t\tformData.append('remember-me', 'on');\n\t\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\tconst response = await fetch('/api/v1/auth/login', {\n\t\t\t\t\t\t\t\t\tmethod: 'POST',\n\t\t\t\t\t\t\t\t\tbody: formData\n\t\t\t\t\t\t\t\t});\n\t\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\tconst result = await response.json();\n\t\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\tif (result.success) {\n\t\t\t\t\t\t\t\t\tthis.message = { text: result.data.message, type: 'success' };\n\t\t\t\t\t\t\t\t\t// Redirect to dashboard\n\t\t\t\t\t\t\t\t\tsetTimeout(() => {\n\t\t\t\t\t\t\t\t\t\twindow.location.href = result.data.redirect;\n\t\t\t\t\t\t\t\t\t}, 1000);\n\t\t\t\t\t\t\t\t} else {\n\t\t\t\t\t\t\t\t\tthis.message = { text: result.error, type: 'error' };\n\t\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\t} catch (error) {\n\t\t\t\t\t\t\t\tthis.message = { text: 'Network error. Please try again.', type: 'error' };\n\t\t\t\t\t\t\t} finally {\n\t\t\t\t\t\t\t\tthis.loading = false;\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t},\n\t\t\t\t\t\t\n\t\t\t\t\t\tvalidateForm() {\n\t\t\t\t\t\t\tlet isValid = true;\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\tif (!this.form.username.trim()) {\n\t\t\t\t\t\t\t\tthis.errors.username = 'Username is required';\n\t\t\t\t\t\t\t\tisValid = false;\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\tif (!this.form.password) {\n\t\t\t\t\t\t\t\tthis.errors.password = 'Password is required';\n\t\t\t\t\t\t\t\tisValid = false;\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\treturn isValid;\n\t\t\t\t\t\t},\n\t\t\t\t\t\t\n\t\t\t\t\t\tclearErrors() {\n\t\t\t\t\t\t\tthis.errors = {};\n\t\t\t\t\t\t\tthis.message = { text: '', type: '' };\n\t\t\t\t\t\t}\n\t\t\t\t\t}\n\t\t\t\t}\n\t\t\t</script><!-- Register link (hidden when registration is disabled) -->")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		if oauthConfig == nil || oauthConfig.RegistrationEnabled {
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 9, "<p class=\"mt-10 text-center text-sm text-gray-500 dark:text-gray-400\">Not registered? <a href=\"/register\" class=\"font-semibold leading-6 text-primary-600 hover:text-primary-500\">Create an account</a></p>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
		}
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 10, "<!-- Dark mode toggle --><div class=\"mt-8 flex justify-center\"><button @click=\"toggle()\" class=\"rounded-lg p-2 text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-dark-bg-secondary transition-colors\"><span x-show=\"!darkMode\">🌙 Dark Mode</span> <span x-show=\"darkMode\">☀️ Light Mode</span></button></div></div></div>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return nil
